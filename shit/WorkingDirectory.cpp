...
void WorkingDirectory::PlaceSnapshotIn(const Snapshot& snapshot)
{
	...
	RemovalThatDontMatch(shit.workingDirectory, shitIgnore);

	for (auto& [obj, path] : snapshot.objects) {
		auto directory = std::filesystem::path(shit.workingDirectory + path).remove_filename();
		std::filesystem::create_directories(directory);
		std::filesystem::copy_file(obj.getPath(), shit.workingDirectory + path);
	}
}


template<typename Func>
void getFilesRecurse(Shit& shit, std::vector<std::pair<File::Key, std::string>>& files, std::string current_directory,
	Func shouldIgnore) {
	for (const auto& entry : std::filesystem::directory_iterator(current_directory)) {
		if (!shouldIgnore(entry)) {
			if (entry.is_directory()) {
				getFilesRecurse(shit, files, entry.path().string(), shouldIgnore);
			}
			else if (entry.is_regular_file()) {
				auto path = entry.path().string();
				path = path.substr(shit.workingDirectory.length());
...
std::vector<std::pair<File::Key, std::string>> WorkingDirectory::getFiles()
{
	...
	getFilesRecurse(shit, files, shit.workingDirectory, shitIgnore);
