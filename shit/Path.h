#pragma once
#include <string>
namespace Shit {
	namespace Path {
		inline std::string workingDirectory;
		inline std::string shitDirectory;
		inline std::string staging;
		inline std::string head;
		inline std::string headRelative = ".shit\\head";

		inline std::string objectsDirectory;
		inline std::string objectsRelative = ".shit\\objects\\";
		inline std::string snapshotDirectory;
		inline std::string snapShotRelative = ".shit\\snapshots\\";
		inline std::string branchesDirectory;
		inline std::string branchesRelative = ".shit\\branches\\";
	}
}
